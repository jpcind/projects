...
# SPECIFICATION: using deeplearning to classify fashion items
    ...
    for j in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for k in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            for m in range(len(a)):
                if a[m] == b[m]:
                    inner_correct += 1
            acc = inner_correct / len(X_test)
            # print("class_predicted {}".format(class_predicted))
            ...
            if highestAccuracy < acc:
                ...
                print("Parameters: " + "Number of Hidden Layers: " + str(i) + ",number of neurons: " + str(j) + ",learning rate: " + str(k))
            print()
print("Highest accuracy: {}".format(highestAccuracy))
...
# and its number of parameters. Note that Dense layers often have a lot of parameters.
# This gives the model quite a lot of flexibility to fit the training data,
# but it also means that the model runs the risk of overfitting,
